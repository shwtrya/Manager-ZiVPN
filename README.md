...
## Update v1.4 (Live Alerts + Scheduler)
- 🔔 Live Alert Telegram (overlimit IP, IP baru, service down/recover, event cron)
- ⏱ Scheduler Center via Telegram (toggle & set jam auto-expire/cleanup/backup)

### Commands (Owner)
- `/addadmin <telegram_id>` / `/deladmin <telegram_id>`
- `/addviewer <telegram_id>` / `/delviewer <telegram_id>`

### Shortcuts
- `/scheduler` (Admin)
- `/alerts` (Admin)

## Update v1.5 (Security Center + Daily Stats + Device Binding)
- 🛡️ Security Center (Firewall hardening safe + Torrent blocker status/apply)
- 📈 Daily Stats (peak/avg online, newip, overlimit, device mismatch, top users)
- 🔐 Device Binding (opsional): 1 akun terikat 1 IP pertama (lebih kuat dari limit IP)

### Shortcuts
- `/security` (Admin)
- `/stats` (Admin)
- `/bindreset <username>` (Admin)
